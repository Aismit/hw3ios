...
func getPosts(user: CurrentUser, completion: @escaping ([Post]?) -> Void) {
    ...
    var parray: [Post] = []
        dref.child("Posts").observeSingleEvent(of: .value, with: { snapshot -> Void in
            if snapshot.exists() {
                if let posts = snapshot.value as? [String:AnyObject] {
                    user.getReadPostIDs(completion: { (ids) in
                        for val in posts.keys {
                            let ip = posts[val]!["imagePath"] as! String
                            let un = posts[val]!["username"] as! String
                            let dt = posts[val]!["date"] as! String
                            let t = posts[val]!["thread"] as! String
                            let rd = ids.contains(val)
                            let n = Post(id: val, username: un, postImagePath: ip, thread: t, dateString: dt, read: rd)
                            parray.append(n)
                        }
                        completion(parray)
                    })
